...
# ============================================================
# VOZ MASCULINA GRAVE: pyttsx3 + PROCESADO + REVERBERACIÓN
# ============================================================
def procesar_voz_ultratumba(wav_path):
    """Hace la voz más grave/gruesa y añade únicamente reverberación suave."""
    try:
        import numpy as np
    ...
    grave = resample_poly(data, 168, 100).astype(np.float32)

    sos = butter(2, 520, btype="lowpass", fs=sr, output="sos")
    low = sosfilt(sos, grave)
    grave = grave * 0.82 + low * 0.18

    grave = np.tanh(grave * 1.12)

    original = grave.copy()
    reverberada = grave.copy()
    for delay_s, gain in [(0.050, 0.18), (0.100, 0.13), (0.160, 0.09), (0.240, 0.06)]:
        d = int(delay_s * sr)
        if d < len(original):
            reverberada[d:] += original[:-d] * gain

    grave = np.tanh(reverberada * 0.98)
    peak = np.max(np.abs(grave)) or 1.0
    grave = grave / peak * 0.88

    fade_in = min(int(0.05 * sr), len(grave)//4)
    fade_out = min(int(0.45 * sr), len(grave)//4)
    if fade_in:
        grave[:fade_in] *= np.linspace(0, 1, fade_in)
    if fade_out:
        grave[-fade_out:] *= np.linspace(1, 0, fade_out)

    return sr, (grave * 32767).astype(np.int16)


def generar_voz_ultratumba(texto, semilla):
    """Genera la misma voz que se reproduce en la página y en el WAV descargable."""
    try:
        import pyttsx3
    except Exception:
        return None
    ...
    try:
        ...
        masculinas = [
            "jorge", "david", "pablo", "carlos", "juan", "miguel",
            "diego", "antonio", "daniel", "enrique", "raul", "raúl",
            "manuel", "fernando", "sergio", "male", "hombre"
        ]
        espanolas = [
            "spanish", "español", "es-mx", "es_es", "es-es", "es_419",
            "es-419", "es-us", "es_us"
        ]
        ...
        for voz in voces:
            blob = meta(voz)
            if any(e in blob for e in espanolas) and any(m in blob for m in masculinas):
                voz_elegida = voz
                break
        ...
        if voz_elegida is None:
            for voz in voces:
                blob = meta(voz)
                if any(m in blob for m in masculinas):
                    voz_elegida = voz
                    break
        ...
        if voz_elegida is None:
            for voz in voces:
                blob = meta(voz)
                if any(e in blob for e in espanolas):
                    voz_elegida = voz
                    break

        if voz_elegida is None and voces:
            voz_elegida = voces[0]

        if voz_elegida is not None:
            engine.setProperty("voice", voz_elegida.id)
        ...
        procesado = procesar_voz_ultratumba(path)
        if procesado is None:
            return None

        sr, samples = procesado
        out = io.BytesIO()
...
if st.session_state.resultado_generado and st.session_state.resultado:
    ...
    if st.session_state.audio_generado:
        ...
        st.markdown("<div class='oraculo-box'>La voz del expediente no pudo ser generada en este equipo.</div>", unsafe_allow_html=True)
